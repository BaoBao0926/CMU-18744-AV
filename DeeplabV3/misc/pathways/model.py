...
from .utils.dataloader import SceneDataset, scene_collate, WorkzoneTrajectoryDataset, workzone_collate, get_data_loader


from .test import evaluate, evaluate_workzone
from .train import train, train_workzone
...
class YNetTorch(nn.Module):
	def __init__(self, obs_len, pred_len, segmentation_model_fp, use_features_only=False, semantic_classes=6,
		...
		if segmentation_model_fp is not None:
			self.semantic_segmentation = torch.load(segmentation_model_fp, weights_only=False)
...
class YNet:
	...
	def train_workzone(self, params, train_image_path, val_image_path, train_data_path, val_data_path,
            experiment_name, batch_size=8, num_goals=1, num_traj=1, 
            device=None, dataset_name=None):
		if device is None:
		...
		train_dataset = WorkzoneTrajectoryDataset(train_image_path, train_data_path, params, total_len)
		train_loader = get_data_loader(train_dataset, batch_size, collate_fn=workzone_collate)

		val_dataset = WorkzoneTrajectoryDataset(val_image_path, val_data_path, params, total_len)
		val_loader = get_data_loader(val_dataset, batch_size, collate_fn=workzone_collate)
		...
		params_not_in_semantic_segmentation = [
			param for name, param in model.named_parameters() if 'semantic_segmentation' not in name
		]

		optimizer = torch.optim.Adam(
			[ 
				{ "params" : params_not_in_semantic_segmentation, "lr" : params["learning_rate"], },
				{ "params" : model.semantic_segmentation.parameters(), "lr" : params["learning_rate"] / 100, },					
			], 
		...
		for e in tqdm(range(params['num_epochs']), desc='Epoch'):
			train_ADE, train_FDE, train_loss = train_workzone(model, train_loader, e, obs_len, pred_len,
													 batch_size, params, gt_template, device,
													 input_template, optimizer, criterion, dataset_name)
			...
			val_ADE, val_FDE = evaluate_workzone(model, val_loader, num_goals, num_traj,
			...
			# if train_ADE < best_test_ADE:
			# 	print(f'Best Epoch {e}: \nVal ADE: {train_ADE} \nVal FDE: {train_FDE}')
			# 	torch.save(model.state_dict(), 'pretrained_models/' + experiment_name + '_weights.pt')
			# 	best_test_ADE = train_ADE
