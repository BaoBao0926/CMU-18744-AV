...
# --- Config (globals) ---
DEFAULT_SOURCES = {
    "ROADwork": os.path.join("Datasets", "ROADwork", "images"),
    "BDD100K": os.path.join("Datasets", "BDD100K", "train"),
}
OUTPUT_ROOTS = {
    "ROADwork": os.path.join("output", "ROADwork"),
    "BDD100K": os.path.join("output", "BDD100K"),
}
...
def _is_image(path):
    return os.path.splitext(path.lower())[1] in {".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff"}


def _is_video(path):
    return os.path.splitext(path.lower())[1] in {".mp4", ".avi", ".mov", ".mkv", ".wmv", ".m4v"}


def _next_exp_dir(root_dir):
    idx = 0
    while True:
        exp_dir = os.path.join(root_dir, f"exp_{idx}")
...
def _process_frame(im0, yolo, deeplab, device, half, transform_full, transform_padded, th_lut):
    ...
    deeplab_vis = run_deeplab(deeplab, device, im0, transform_full, transform_padded, th_lut)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ROADwork demo for images and videos")
    parser.add_argument("--datasets", type=str, default="ROADwork", choices=["ROADwork", "BDD100K"], help="Dataset name")
    parser.add_argument("--source", type=str, default=None, help="Image/video file or folder")
    args = parser.parse_args()

    device = select_device("0")
    source = args.source or DEFAULT_SOURCES[args.datasets]
    output_root = OUTPUT_ROOTS[args.datasets]
    ...
    if os.path.isdir(source):
        image_out_dir = os.path.join(output_root, "image")
        video_out_dir = os.path.join(output_root, "video")
        os.makedirs(image_out_dir, exist_ok=True)
        os.makedirs(video_out_dir, exist_ok=True)
        entries = [os.path.join(source, f) for f in sorted(os.listdir(source))]
        image_paths = [p for p in entries if os.path.isfile(p) and _is_image(p)]
        video_paths = [p for p in entries if os.path.isfile(p) and _is_video(p)]

        for path in tqdm(image_paths, desc="ROADwork images"):
            im0 = cv2.imread(path)
            if im0 is None:
                continue
